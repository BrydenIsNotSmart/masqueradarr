...
// gate allows *.dulo.gd plus any host LEARNED from a playlist we legitimately resolved/fetched
...
// Hosts allowed for direct (non-entry) proxy hops. *.dulo.gd is static; additional playbackUrl hosts are
...
const duloAdapter: SourceAdapter = {
  ...
  async listChannels() {
    try {
      const res = await fetch(DULO_API, {
        headers: {
          "User-Agent": UA,
          Origin: DULO_ORIGIN,
